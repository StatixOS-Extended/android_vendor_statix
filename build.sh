...
function showHelpAndExit {
        ...
        echo -e "${CLR_BLD_BLU}  -v, --variant         STATIX variant - Can be alpha, beta or release${CLR_RST}"
...
while true; do
    case "$1" in
        ...
        -v|--variant|v|variant) STATIX_VARIANT="$2"; shift;;
        ...
        -n|--version|n|version) STATIX_USER_VERSION="$2"; shift;;
        --) shift; break;;
    esac
    shift
done
...
if [ ! -d "$DIR_ROOT/vendor/statix" ]; then
        echo -e "${CLR_BLD_RED}error: insane root directory ($DIR_ROOT)${CLR_RST}"
        exit 1
fi

# Setup STATIX variant if specified
if [ $STATIX_VARIANT ]; then
    STATIX_VARIANT=`echo $STATIX_VARIANT |  tr "[:upper:]" "[:lower:]"`
    if [ "${STATIX_VARIANT}" = "release" ]; then
        export STATIX_BUILDTYPE=RELEASE
    elif [ "${STATIX_VARIANT}" = "beta" ]; then
        export STATIX_BUILDTYPE=BETA
    elif [ "${STATIX_VARIANT}" = "alpha" ]; then
        export STATIX_BUILDTYPE=ALPHA
    else
        echo -e "${CLR_BLD_RED} Unknown STATIX variant - use beta or release${CLR_RST}"
        exit 1
    fi
fi

# Setup STATIX version if specified
if [ $STATIX_USER_VERSION ]; then
    # Check is it a number
    if [ ! -z "${STATIX_USER_VERSION##*[!0-9]*}" ]; then
        export STATIX_BUILDVERSION=${STATIX_USER_VERSION}
    else
        echo -e "${CLR_BLD_RED} Invalid STATIX minor version - use any non-negative integer${CLR_RST}"
        exit 1
    fi
fi
...
STATIX_DISPLAY_VERSION="$(cat $DIR_ROOT/vendor/statix/target/product/version.mk | grep 'STATIX_MAJOR_VERSION := *' | sed 's/.*= //') "
if [ $STATIX_BUILDVERSION ]; then
    STATIX_DISPLAY_VERSION+="$STATIX_BUILDVERSION"
else
    STATIX_DISPLAY_VERSION+="$(cat $DIR_ROOT/vendor/STATIX/target/product/version.mk | grep 'STATIX_MINOR_VERSION := *' | tail -n 1 | sed 's/.*= //')"
fi
...
echo -e "${CLR_BLD_GRN}Building STATIX $STATIX_DISPLAY_VERSION for $DEVICE${CLR_RST}"
...
echo -e ""
lunch "statix_$DEVICE-$BUILD_TYPE"
STATIX_VERSION="$(get_build_var STATIX_VERSION)"
checkExit
echo -e ""
...
# Build signed rom package if specified
    ...
    sign_target_files_apks -o -d $KEY_MAPPINGS \
        "$OUT"/obj/PACKAGING/target_files_intermediates/statix_$DEVICE-target_files-$FILE_NAME_TAG.zip \
        statix-$STATIX_VERSION-signed-target_files-$FILE_NAME_TAG.zip
    ...
    if [ "$DELTA_TARGET_FILES" ]; then
        ...
        ota_from_target_files -k $KEY_MAPPINGS/releasekey \
            ...
            statix-$STATIX_VERSION-signed-target_files-$FILE_NAME_TAG.zip \
            statix-$STATIX_VERSION-delta.zip
        checkExit
    fi
    ...
    if [ "$FLAG_IMG_ZIP" = 'y' ]; then
        ...
        img_from_target_files \
            statix-$STATIX_VERSION-signed-target_files-$FILE_NAME_TAG.zip \
            statix-$STATIX_VERSION-image.zip
        checkExit
    fi
# Build rom package
    ...
    cp -f $OUT/statix_$DEVICE-ota-$FILE_NAME_TAG.zip $OUT/statix-$STATIX_VERSION.zip
    echo "Package Complete: $OUT/statix-$STATIX_VERSION.zip"
fi
echo -e ""
